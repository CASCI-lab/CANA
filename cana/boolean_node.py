...
class BooleanNode(object):
    ...
    def _input_symmetry(self, accumOp, kernel):
        self._check_compute_canalization_variables(ts_coverage=True)

        summand = []
        for fAlpha, fTheta in self._ts_coverage.items():
            summand.append(accumOp(list(map(kernel, fTheta))))
        return np.mean(summand)

    def input_symmetry(self, op="mean", kernel="numDots", sameSymbol=False):
        ...
        return self._input_symmetry(strToOp[op], kernFunc)
